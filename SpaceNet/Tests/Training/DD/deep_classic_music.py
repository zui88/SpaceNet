...
from Utils.DeepAugmented.LossFunctions.rmse_loss import RMSELoss
...
def train():
    ...
    R_train, R_test, dd_train, dd_test = train_test_split(R, dd, test_size=0.12, random_state=42)

    engine                           = create_engine(config, define_models=True)
    history: keras.callbacks.History = engine.capability_registry[DeepAugment].train_models(
        R_train,
        dd_train,
        (R_test, dd_test),
        batch_size=100,
