from SpaceNet.Capabilities.deep_augment import (
    ModelName,
    Dataset,
    Percent,
    TransformerClass,
...
class DeepAugmentV1:
    def __init__(
        ...
        self.paths = [
            base_path + "/" + version + "_" + base_name + "_" + name + ".keras"
            for name in network_names
        ]
        self.name_paths: dict[ModelName, Path] = {
            name: Path(path) for name, path in zip(network_names, self.paths)
        }
        self.models: dict[ModelName, keras.models.Model] | None = None
        self._transformer: Callable[[Dataset], Dataset] | TransformerClass | None = (
            lambda x: x
        )
    ...
    def transformer(self) -> Callable[[Dataset], Dataset] | None:
        ...
        if hasattr(self._transformer, "transform"):
            if (
                hasattr(self._transformer, "is_initialized")
                and not self._transformer.is_initialized()
            ):
                if hasattr(self._transformer, "load"):
                    self._transformer = self._transformer.load(
                        self.workdir + "/" + "scaler.npz"
                    )
    ...
    def load_models(self):
        self.models = {
            name: keras.models.load_model(path)
            for name, path in self.name_paths.items()
        }
    ...
    def save_models(self):
        if self.models is not None:
            ...
            if hasattr(self._transformer, "save"):
                self._transformer.save(self.workdir + "/" + "scaler.npz")
        else:
