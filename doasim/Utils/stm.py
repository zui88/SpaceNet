...
class StateMachine[S: Enum, E: Enum, C]:
    ...
    def add_transition(
        ...
        """
        Add transition to the generic state machine with the action function.
        """
        self.transitions[(from_state, event)] = (to_state, func)

    def transition(self, from_state: S | Iterable[S], event: E, to_state: S):
        """
        Add transition decorator for registering the action function.
        """
    ...
    def handle(self, ctx: C, state: S, event: E) -> S:
        next_state, action = self._next_transition(state, event)
        action(ctx)
        return next_state

    def _next_transition(self, state: S, event: E) -> tuple[S, Action[C]]:
        try:
