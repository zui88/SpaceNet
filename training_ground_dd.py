from doa import DeepDopplerMusic, RMSELoss
from generator import (
    ChirpSignal,
    DelayDopplerSignalSynthesizer,
    ObservationContext,
    RandomArray,
)
...
def model_train():
    ...
    - observation context (length of observation)
    """
    
    model_name       = "modelv1_dd_"
    suffix           = ".keras"
    ...
    # dd -> delay, doppler
    R, dd = generate_training_set(
        signal_generator=signal_generator,
        training_examples=200,
        delay_range=(0.8, 7.0),
        max_signal_sources=signal_sources,
        min_signal_sources=signal_sources,
        snr_db_range=(10.0, 40.0),
        seed=42,
        observ_ctx=observ_ctx,
    )
    R_train, R_test, dd_train, dd_test = train_test_split(R, dd, test_size=0.1)
    ...
    history = music_engine.fit(
        ...
        epochs=60,
        batch_size=32,
        validation_data=(R_test, dd_test),
        callbacks=[
            reduce_lr,
            stop_fit
        ],
